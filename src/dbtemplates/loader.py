...
from dbtemplates.utils.cache import cache, get_cache_key, get_cache_notfound_key, set_and_return
...
class Loader(BaseLoader):
    ...
    def _load_template_source(self, template_name, template_dirs=None):
        ...
        try:
            return self._load_and_store_template(template_name, cache_key, site, sites__in=[site.id])
        except (Template.MultipleObjectsReturned, Template.DoesNotExist):
            try:
                return self._load_and_store_template(template_name, cache_key, site, sites__isnull=True)
            except (Template.MultipleObjectsReturned, Template.DoesNotExist):
                pass
