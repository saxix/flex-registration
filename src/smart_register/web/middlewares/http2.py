...
def create_preload_header(urls, nonce=None, server_push=None):
    ...
    preload_tags = (
        ...
        if ext in PRELOAD_AS
        else f"<{url}>; rel=preload; crossorigin{nonce}{nopush}"
...
def get_cached_preload_urls(request):
    ...
    if getattr(settings, "HTTP2_PRESEND_CACHED_HEADERS", False) and request.path in cached_preload_urls:
...
def set_cached_preload_urls(request):
    ...
    if getattr(settings, "HTTP2_PRESEND_CACHED_HEADERS", False) and getattr(request, "to_preload", None):
...
def late_preload_response(request, get_response, nonce):
    ...
    if getattr(request, "to_preload", None):
...
#         if should_preload(request):
#             return preload_response(request, self.get_response)
#         return no_preload_response(request, self.get_response)
#
