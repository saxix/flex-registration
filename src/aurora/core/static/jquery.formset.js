...
;(function ($) {
    ...
    $.fn.formset = function (opts) {
        ...
        $$.each(function (i) {
            ...
            // + "\" value=\"on\" />"); row.hide(); } else { del.before("<input type=\"hidden\" name=\"" +
            // del.attr("name") + "\" id=\"" + del.attr("id") + "\" />"); } // Hide any labels associated with the
            // DELETE checkbox: $("label[for=\"" + del.attr("id") + "\"]").hide(); del.remove(); }
    ...
    $.fn.formset.defaults = {
        ...
        keepFieldValues: "",             // jQuery selector for fields whose values should be kept when the form is
                                         // cloned
        added: null,                     // Function called each time a new form is added
        initilized: null,                // Function called when formses is initilized
        removed: null,                   // Function called each time a form is deleted
        hideLastAddForm: false           // When set to true, hide last empty add form (becomes visible when clicking
                                         // on add button)
    };
})(jQuery);
