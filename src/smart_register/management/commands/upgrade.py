...
@click.option(
    '--admin-email', '-e', default='', envvar='ADMIN_EMAIL', help='Admin email'
)
@click.option(
    '--admin-password',
    '-p',
...
@click.option(
    '--migrate/--no-migrate', default=True, is_flag=True, help='Run database migrations'
)
@click.option(
    '--static/--no-static', default=True, is_flag=True, help='Collect static assets'
)
@click.option(
    '--init-stripe', default=True, is_flag=True, help='Upload price list to Stripe'
)
def upgrade(
    admin_email,
    admin_password,
    ...
    **kwargs
    ...
    vf1, __ = Validator.objects.update_or_create(
        ...
        defaults={
            'message': "name must start with 'S'",
...
dt > limit;""",
    ...
    hh, __ = FlexForm.objects.get_or_create(
        name='Household', defaults=dict(validator=vf1)
    )
    ...
    ind.fields.get_or_create(
        label='First Name',
        defaults=dict(field=forms.CharField, required=True, validator=v1),
    )
    ind.fields.get_or_create(
        label='Last Name', defaults=dict(field=forms.CharField, validator=v1)
    )
    ind.fields.get_or_create(
        label='Date Of Birth', defaults=dict(field=forms.DateField, validator=v2)
    )

    ind.fields.get_or_create(
        label='Options',
        defaults=dict(field=forms.ChoiceField, choices='opt 1, opt 2, opt 3'),
    )

    hh.formsets.get_or_create(name='individuals', defaults=dict(flex_form=ind))

    DataSet.objects.get_or_create(name='Registration1', defaults=dict(flex_form=hh))
